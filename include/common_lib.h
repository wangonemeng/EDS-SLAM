...
typedef point_livo::Pose6D Pose6D;
...
struct MeasureGroup {
  ...
  std::deque<struct ImgIMUsGroup> img_imus;
  int lidar_scan_index_now;
  MeasureGroup() {
    lidar_beg_time = 0.0;
    this->lidar.reset(new PointCloudXYZI());
    std::deque<struct ImgIMUsGroup>().swap(this->img_imus);
  ...
  void debug_show() {
    ...
    for (auto it = this->img_imus.begin(); it != this->img_imus.end(); ++it, ++i) {
      std::cout << "In " << i << " img_imus: ";
      for (auto it_meas = it->imu.begin(); it_meas != it->imu.end(); ++it_meas) {
        std::cout << setprecision(20) << (*it_meas)->header.stamp.toSec() << " ";
      }
      std::cout << "img_time:" << setprecision(20) << this->lidar_beg_time + it->img_offset_time << endl;
      std::cout << "imus_only:" << it->imus_only << "lidar_end_time:"
                << this->lidar_beg_time + this->lidar->points.back().curvature / double(1000) << endl;
    }
